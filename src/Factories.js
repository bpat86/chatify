...
* @return {string} users names concatenated by a ',' or "No users in chat" if no users
*/
const createChatNameFromUsers = (users, excludedUserData = "", disconnectedUserData = "Users") => {
	const excludedUser = users.filter(u => u !== excludedUserData).join(', ');
	const disconnectedUser = disconnectedUserData.substr(0, disconnectedUserData.indexOf(','));

	return excludedUser || disconnectedUser + " left the chat";
}
